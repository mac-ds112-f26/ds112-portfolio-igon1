...
![](https://upload.wikimedia.org/wikipedia/commons/6/68/Map_of_USA_with_county_outlines_%28black_%26_white%29.png) US Counties. Source: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Map_of_USA_with_county_outlines_(black_%26_white).png)
...
>45% of the counties the republican candidate win in 2020. 
...
```{r}
elections |> 
  ggplot(aes(y = county_name, x = repub_pct_20)) +
   geom_point() 
```
...
```{r}
elections |> 
  ggplot(aes(y = repub_pct_20 , x = repub_pct_16)) +
   geom_point() +
    theme_minimal()
...
> Texas tends to have the most counties with outliers. Texas has a higher population density in the area. 
...
> The republican variable and the median variables show the relationship as the median age increases and the median rent derecreases as the republican choice decreases for both of the variables. The republican variable is the best choice for predictor variable. 
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
```
> It helps answer what types of states voted for each candiate and a scatter plot would only show numerical points of the data that won't present a clear value of the state of each voted for. This type of bar provides a better visualization for that. 
...
> In the blue state mostly 55% of the people voted for the republican candiadate and it increases as the pruple state and red sate voted for the candiadete at a county level. 
...
```{r}
# The colors doesn't match to the key and it's not filled in. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r}
# What does scale_fill_manual do? It fills each plot to it's specific data type. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r}
# What does alpha = 0.5 do? it lowers the opacity of the graph
...
```{r}
# What does facet_wrap do?! it seperates each legend data into it's own seperate graph.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# Why is this terrible? Tjis is terrible because it too much going on with the graph. 
...
The transparent density plot graph. 

- What is one pro of density plots relative to boxplots?
One pro is show the numerical variables clealy of mutilplie variables over time. 

- What is one con of density plots relative to boxplots?
The opacity of the desnity plot could be hard to see from the reader's persecptive. 
...
> One pro about this bar plot is it shows the a clear bar plot for each data legend vaue while a con is there no clear showing of correlation between each data value to see how they are related compare to the other three options. 

#### Part b

What's your favorite bar plot from part and why?

> Density plot from Execrise #9 it shows the different values from each one speficic. 
